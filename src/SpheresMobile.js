...
function Sphere(props) {
  return (
    <mesh castShadow {...props} renderOrder={-2000000}>
      ...
      <meshStandardMaterial metalness={0.5} color="grey" />
    </mesh>
  )
}

export default function SpheresMobile() {
  const { height } = useThree((state) => state.viewport)

  const group = useRef()
  useFrame((state) => {
    group.current.children[0].position.x = THREE.MathUtils.lerp(group.current.children[0].position.x, -5 - state.mouse.x * 5, 0.01)
    ...
    group.current.children[11].position.x = THREE.MathUtils.lerp(group.current.children[11].position.x, 5 + state.mouse.x * 5, 0.05)
  })
  return (
    <group ref={group}>
      <Sphere position={[30, 10, -5]} scale={3} /> {/* 1 */}
      <Sphere position={[-2, 0, 0]} scale={2} /> {/* 1 */}
      <Sphere position={[4, 0, 1]} scale={1.25} /> {/* Good */}
